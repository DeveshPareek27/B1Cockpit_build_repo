...
set "APP_PATH=%WORKSPACE%\app"

set "UI5_PATH=%APP_PATH%\UI5"

set "API_PATH=%APP_PATH%\API"

set "CUSTOM_NODE_PATH=%WORKSPACE%\custom-nodes"
...
set "ELECTRON_SCRIPT_PATH=%ROOT_DIR%\"
...
REM IMPORTANT:
REM If flow.json exists in workspace/app/flow.json then use APP_PATH
REM If flow.json exists in workspace/app/UI5/flow.json then use UI5_PATH

set "FLOW_JSON_PATH=%APP_PATH%\flow.json"
...
xcopy /s /e /y "%CUSTOM_NODE_PATH%\ENT_RED_CustomNodes\*" "%BUILD_DIR%\API\ENT_RED_CustomNodes\" >nul
...
if exist "%ELECTRON_SCRIPT_PATH%electron_files\main_b1Cockpit_template.js" (
    ...
    echo [ERROR] main_b1Cockpit_template.js not found. Exiting...
    exit /b 1
)
...
REM ===================================================================
REM SCRIPT 11: COPY INSTALLER TO OUTPUT
REM ===================================================================
...
xcopy /E /I /Y "%ELECTRON_SCRIPT_PATH%\%PROJECT%-Installer-%version%.exe" "%OUTPUT_DIR%" >nul
